...
CREATE TABLE IF NOT EXISTS dw.dim_estacion (
    ...
    estado_operativo VARCHAR(50)     NOT NULL DEFAULT 'Activo'
                     CHECK (estado_operativo IN ('Activo', 'Inactivo', 'Mantenimiento', 'Desconocido'))
);
...
CREATE TABLE IF NOT EXISTS dw.fact_evento_sismico (
    ...
    id_ubicacion    INT             NOT NULL
                    REFERENCES dw.dim_ubicacion(id_ubicacion) ON DELETE RESTRICT,
    id_tiempo       INT             NOT NULL
                    REFERENCES dw.dim_tiempo(id_tiempo)       ON DELETE RESTRICT,
    id_estacion     INT
                    REFERENCES dw.dim_estacion(id_estacion)   ON DELETE SET NULL,
    -- Métricas del evento
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'etl_loader') THEN
        CREATE ROLE etl_loader LOGIN PASSWORD 'CHANGE_IN_PRODUCTION';
    END IF;
END
$$;
...
GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA dw TO etl_loader;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA dw TO etl_loader;
...
INSERT INTO dw.dim_ubicacion (latitud, longitud, zona_geografica) VALUES
    (9.9281,  -84.0907, 'Valle Central'),
    (10.4634, -85.1396, 'Guanacaste'),
    (9.0000,  -83.3500, 'Zona Sur'),
    (10.0000, -83.0000, 'Caribe'),
    (9.6500,  -84.6500, 'Pacífico Central')
ON CONFLICT DO NOTHING;

-- Mensaje de confirmación en los logs del contenedor
DO $$ BEGIN
    RAISE NOTICE '✅ Esquema estrella RSN Data Warehouse inicializado correctamente.';
END $$;
